...
def answer(heights):
    max_from_left = cumulative_max(heights)
    max_from_right = reversed(cumulative_max(reversed(heights)))
    water_heights = map(min, zip(max_from_left, max_from_right))
    diff = map(lambda x: x[1] - x[0], zip(heights, water_heights))
    return sum(diff)
